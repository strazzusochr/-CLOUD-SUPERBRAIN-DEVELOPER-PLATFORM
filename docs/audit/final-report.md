...
- **Backend layers (ORC/AP/LLM/MCP/MEM/OBS): PASS (local dry-run runtime)** — `docker-compose.dev`
  ...
  `deterministic_dry_run` (`live_provider_calls: false`). **All six runtime services healthy
  (MEM PASS** — memory-worker rebuilt from current code, heartbeat live). Evidence:
  `docs/audit/backend-runtime-evidence.md`. Hosted prod is separate/gated.
- **Live data across the operational pages** — `/organism`, `/observe`, `/diagnostics`, `/files`,
  `/agents`, `/tools`, `/evidence`, `/home` all project real local-runtime data (live-state,
  metrics, project-progress, pgvector count, agent roster, cloud readiness, gate/service health)
  with honest spec/mock fallback on Vercel. No secret/token value is ever surfaced.
- **Real GLB core asset** — `/public/organism/core.glb` (1280-face crystalline icosphere,
  generated by `scripts/gen-core-glb.js`) loaded via `useGLTF`; basic path verifiable headless,
  full PBR on hardware. e2e asserts the asset + glTF magic + canvas fetch.
- **Audit-hardened** — a 7-agent quality workflow's 19-item punch-list (fake-live, responsive
  dead zone, 3D state colour, a11y, fetch timeouts) applied + verified.
