...
- Cloud provider inventory: `/api/v1/clouds` must return `cloud-provider-inventory-v1` with evidence `cloud_provider_inventory_visible`, including read-only Vercel, Hetzner, Cloudflare, GitHub, GHCR, Hugging Face, GitLab, and GitKraken provider surfaces when their tokens are configured.
...
- Vercel backend origins: `AGENT_API_BASE_URL`, `MCP_GATEWAY_BASE_URL`, and `LLM_GATEWAY_BASE_URL` must be HTTPS non-local origins and must pass health probes.
...
- `production_deploy_claim_allowed=true` requires hosted staging, cloud deployment preflight visibility, current branch-protection verification, GHCR image digest proof, Vercel backend origin health, canonical gitleaks, and Hetzner live budget proof.
...
- No Hetzner live infrastructure state is current unless a real Hetzner token is configured for the check.
