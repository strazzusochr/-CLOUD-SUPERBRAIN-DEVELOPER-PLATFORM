...
Contract version: `external-gate-audit-v2`

Canonical runtime mirror: `external-gate-summary-v2`
...
- Cloud provider inventory: `/api/v1/clouds` must return `cloud-provider-inventory-v1` with evidence `cloud_provider_inventory_visible`, including the active Vercel, Cloudflare-native, GitHub, GHCR, Hugging Face, GitLab, and Grafana Cloud provider surfaces when their tokens are configured. Fly.io is historical-only provenance.
...
- `production_deploy_claim_allowed=true` requires hosted staging, cloud deployment preflight visibility, current branch-protection verification, GHCR image digest proof, Vercel backend origin health, canonical gitleaks, and the Cloudflare-native zero-card hosted runtime gate.
...
- Full diagnostic run artifacts are non-secret ignored JSON files under `.phase1-artifacts/`; the sanitized tracked authority is `docs/runtime-state/external-gate-audit-v2.json`.
...
- The former `fly_live_budget_check` and `FLY_API_TOKEN` path are `historical_only` provenance and cannot close an active gate.
