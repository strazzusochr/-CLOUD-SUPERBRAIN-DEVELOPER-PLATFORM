...
function nativeWorkersAiSseWithOpenAiRoleAndTerminal(content = "verified") {
  const base = { id: "chatcmpl-provider-stream", object: "chat.completion.chunk", created: 1_788_000_000, model: primaryModel };
  return [
    `data: ${JSON.stringify({ response: content })}\n\n`,
...
test("stream accepts an OpenAI role metadata frame before the terminal after native Workers AI content", async () => {
  const AI = aiBinding([{ stream: streamFrom(nativeWorkersAiSseWithOpenAiRoleAndTerminal("verified")) }]);
  const DB = auditDb();
  const environment = runtimeEnv(AI, { DB });
  const response = await worker.fetch(
    chatRequest(completionBody({ stream: true }), { headers: { "x-request-id": "stream-native-workers-ai-openai-role-test" } }),
    environment,
  );
  const sse = await readSse(response);
  assert.equal(response.status, 200);
  ...
  assert.equal(sse.data.at(-1), "[DONE]");
  assert.equal(DB.writes.length, 1);
  const proofResponse = await evidenceReadback(environment, "stream-native-workers-ai-openai-role-test", responseTraceId(response));
  const proof = await proofResponse.json();
  assert.equal(proofResponse.status, 200);
  assert.equal(proof.provider_stream_frame_format, "workers_ai_native_response");
  assert.equal(proof.provider_stream_terminal_mode, "provider_done_marker");
