...
Current honesty guardrail: active candidate `prod-candidate-2026-07-13-local-rc1` is a locally verified backend preparation artifact bound to source commit `c451fa8ff2b631685ad07ebcfcf4dc4a5b418e81`; its six Docker image identities are local only and the planned GHCR tags are unpublished. Separately, the frontend and the stateless read-only Backend Contract Origin are deployed on Vercel. The frontend has a source-bound 22x2 Google Chrome proof. The canonical reproducible standard cloud audit is `blocked`; only an owner-assisted, non-current candidate audit is `verified`. This does not prove the stateful Docker backend stack, Owner release approval, registry publication, full-platform deployment, or release promotion.
