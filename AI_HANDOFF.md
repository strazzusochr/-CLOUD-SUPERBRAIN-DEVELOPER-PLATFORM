...
Current honesty guardrail: active candidate `prod-candidate-2026-07-21-local-rc3` is a locally verified backend preparation artifact bound to current runtime source commit `90b57ecaa54e0ab750a57d0e1acfb33779675f5a`; its six Docker image identities are local only and the planned GHCR tags are unpublished. Runtime-only and full-browser candidate verification artifacts are isolated. The verifier proves committed runtime-source parity, binds rollback to RC2 `1d8304456a6a95a2a05de65cf0d576ee68c20733`, and reports `candidate_technical=true`, `runtime_source_parity=true`, `promotion_eligible=false`. Separately, the frontend and the stateless read-only Backend Contract Origin are deployed on Vercel. The frontend has a source-bound 22x2 Google Chrome proof. The canonical reproducible standard cloud audit is `blocked`; only an owner-assisted, non-current candidate audit is `verified`. This does not prove the stateful Docker backend stack, Owner release approval, registry publication, full-platform deployment, or release promotion.
...
Latest Phase 5 boundary hardening: RC3 `prod-candidate-2026-07-21-local-rc3` rebuilt six images from the clean Git archive of `90b57ecaa54e0ab750a57d0e1acfb33779675f5a` after fixing four invalid GHCR service build contexts. `scripts/verify-phase5-production-candidate-local.ps1` proves OCI/source identity, embedded hashes, Frontend `BUILD_ID`, committed runtime-source parity, RC2 rollback identity, read-only methods, and a real Diagnostics Chromium click. `scripts/verify-current-release-candidate.ps1` now distinguishes the source-bound Vercel deployment snapshot (`overall=84`) from the current local manifest (`overall=86`) instead of requiring a false equality. Focused verification passed with `candidate_technical=true`, `runtime_source_parity=true`, `promotion_eligible=false`, canonical `blocked`. Evidence: `.codex/runs/CURRENT/master-goal/phase5/production-candidate-local`; no stateful parity, deployment, registry publication, percentage increase, or provider write is implied.
