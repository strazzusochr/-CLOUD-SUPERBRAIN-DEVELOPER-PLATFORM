...
Current D3 external gate mirror: `.phase1-artifacts/external-gate-audit-20260710-212315.json` is the current external audit artifact from `external-gate-summary-v1`. It remains blocked with missing/failed gates `hosted_agent_api_contracts`, `github_branch_protection_current_verify`, `vercel_backend_origin_health`, and `fly_live_budget_check`; `canonical_gitleaks_claim_allowed=true` and `ghcr_image_digest_claim_allowed=true` while hosted, production, branch-protection, Fly-budget, and Vercel-origin claims remain blocked. GitLab, Hugging Face, and Grafana identity checks are fail-closed in this no-token baseline. Current hosted proof requires Vercel HTTPS `STAGING_BASE_URL` plus reachable Fly origins.
