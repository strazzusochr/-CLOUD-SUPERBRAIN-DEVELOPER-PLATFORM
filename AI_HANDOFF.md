...
Current honesty guardrail: active candidate `prod-candidate-2026-07-22-local-rc5` is a locally verified backend preparation artifact bound to auth/security source commit `255e328a76b3f84bf74358bc7258b9ffb797b339`; its six Docker image identities are local only and the planned GHCR tags are unpublished. Runtime-only and full-browser candidate verification artifacts are isolated. The verifier proves committed runtime-source parity, binds rollback to RC4 `0679f6ffda099a6fcddf6830839a195ebe7d13a7`, and reports `candidate_technical=true`, `runtime_source_parity=true`, `promotion_eligible=false`. Separately, the frontend and the stateless read-only Backend Contract Origin are deployed on Vercel. The frontend has a source-bound 22x2 Google Chrome proof. The canonical reproducible standard cloud audit is `blocked`; only an owner-assisted, non-current candidate audit is `verified`. This does not prove the stateful Docker backend stack, Owner release approval, registry publication, full-platform deployment, or release promotion.

RC5 covers the committed Phase-3 auth and frontend dependency security repair described below. Current runtime-source parity is locally verified; DEV-ONLY; hosted proof still blocked.
...
Latest Phase 5 boundary hardening: RC5 `prod-candidate-2026-07-22-local-rc5` rebuilt six images from the clean Git archive of auth/security commit `255e328a76b3f84bf74358bc7258b9ffb797b339`. `scripts/verify-phase5-production-candidate-local.ps1` proves OCI/source identity, embedded hashes, Frontend `BUILD_ID`, committed runtime-source parity, RC4 rollback identity, read-only methods, and a real Diagnostics Chromium click. `scripts/verify-current-release-candidate.ps1` distinguishes the source-bound Vercel deployment snapshot (`overall=84`) from the current local manifest (`overall=86`) instead of requiring a false equality. Focused verification passed with `candidate_technical=true`, `runtime_source_parity=true`, `promotion_eligible=false`, canonical `blocked`. Evidence: `.codex/runs/CURRENT/master-goal/phase5/production-candidate-local` (candidate SHA-256 `906C202FF2A74D32E52DD3AD77D17910B7777C97C4B3A90A1A761A88603E54B9`, verification SHA-256 `BEA1898986560DE6B987B01752AA2A802EA8209FE171034FFB0E30FD03867EBC`). DEV-ONLY; hosted proof still blocked. No deployment, registry publication, percentage increase, or provider write is implied.
