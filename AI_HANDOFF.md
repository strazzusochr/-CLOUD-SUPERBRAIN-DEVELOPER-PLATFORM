...
Latest bounded proof: `frontend-hosted-current-proof-v1` verifies immutable Vercel preview `dpl_J7BC3uPPUcQBgZckU29kKHXc5Wm9` from source `2e0f57179956ad88657567be65ebe33f1da0d255` and archive SHA-256 `deae678aafe375251023401c06c5b65e8891c3350e542e51ecf8413aaff0253b`. Real Google Chrome `148.0.7778.96` opened all 22 routes by command-palette clicks at desktop and mobile for 44 clicks total; overflow failures, overlay collisions, visible not-found states, and console errors were zero, and four screenshots are present. Evidence is under `.codex/runs/CURRENT/master-goal/frontend/hosted-22x2-2e0f5717-preview-chrome`; configuration and non-claims live in `docs/runtime-state/frontend-hosted-current.json`. Frontend remains `100%`; Overall remains `84%`. This proves the preview, not Production Alias parity, release promotion, or a full-platform production release.

Latest hosted backend boundary proof: `backend-hosted-current-proof-v1` binds immutable Vercel preview `dpl_32rFKVF1W4rkVqq6rPhPsqtPvXEZ` to the same source and archive hash. Authenticated read-only Vercel metadata proves READY state, target `preview`, and exact metadata. Direct unsafe requests remain protected with HTTP 401; the authenticated automation probe reaches the application and proves the deployment-embedded source snapshot at `overall=84`, `P4=100`, integrity `verified`, external gates `5/6 action_required`, summary `blocked`, only blocker `hosted_backend_origins`, MCP/LLM `healthy`, expected stateless Agent API `degraded`, and POST fail-closed with HTTP 503. The current local canonical standard is tracked separately by audit `20260720-191532` at `5/6`. Evidence is `.codex/runs/CURRENT/master-goal/backend/hosted-contract-origin-2e0f5717-preview/verification.json`; configuration and non-claims live in `docs/runtime-state/backend-hosted-current.json`. This is not the stateful Docker backend stack, Production Alias parity, or a full-platform production release.

Latest Phase 5 boundary hardening: `scripts/verify-current-release-candidate.ps1` resolves a missing or inherited retired staging URL to the verified read-only Backend Contract Origin and asserts its read-only, non-stateful, non-production flags before network reads. An explicitly supplied `sslip.io` URL remains rejected. For this immutable fallback, the hosted payload must match the deployment snapshot recorded in `backend-hosted-current.json`; the current local canonical summary remains the sole promotion authority. `scripts/manual/verify-phase5-staging-immutable-parity.ps1` passes isolated `plan-only.invalid` values into the immutable deployment-plan check, so stale provider variables cannot contaminate it. Focused verification passed with `candidate_technical=true`, `promotion_eligible=false`, canonical `blocked`; no stateful parity, deployment, registry publication, or provider write is implied.
...
Latest T4 frontend provider-boundary proof: the frontend no longer imports or calls Neon, Cloudflare Workers AI/D1/Vectorize, or the GitHub Store directly. Five retired provider modules were removed; mutation and persistence routes now cross only the configured Agent API, LLM Gateway, or MCP Gateway boundary and fail closed when unavailable. Read projections fall back honestly when hosted state is unavailable, and the Artifact Library effect dependency is stable. Lint, production build, `npm run verify`, `npm run verify:runtime`, and the exact-source `npm run verify:browser` passed; the latter covered all seven Phase-6 gates plus 22 routes, two viewports, and 44 clicks. The static report is `.codex/runs/CURRENT/master-goal/t4/frontend-provider-boundary/report.json`; the source-bound hosted Chrome report is `.codex/runs/CURRENT/master-goal/frontend/hosted-22x2-2e0f5717-preview-chrome`. Overall remains `84%`; no production promotion, live provider write, stateful hosted backend, or percentage increase is claimed.
...
4. Treat `.phase1-artifacts/hosted-browser-proof-20260504-235540.md` as historical provenance only. Current frontend evidence is the fresh Google Chrome preview report under `.codex/runs/CURRENT/master-goal/frontend/hosted-22x2-2e0f5717-preview-chrome`; Production Alias parity is not claimed.
