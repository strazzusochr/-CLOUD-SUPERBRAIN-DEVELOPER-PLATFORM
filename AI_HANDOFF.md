...
Latest Phase 5 boundary hardening: RC4 `prod-candidate-2026-07-21-local-rc4` rebuilt six images from the clean Git archive of `0679f6ffda099a6fcddf6830839a195ebe7d13a7` after the hosted LLM 503 repair and frontend fallback hardening. `scripts/verify-phase5-production-candidate-local.ps1` proves OCI/source identity, embedded hashes, Frontend `BUILD_ID`, committed runtime-source parity, RC3 rollback identity, read-only methods, and a real Diagnostics Chromium click. `scripts/verify-current-release-candidate.ps1` distinguishes the source-bound Vercel deployment snapshot (`overall=84`) from the current local manifest (`overall=86`) instead of requiring a false equality. Focused verification passed with `candidate_technical=true`, `runtime_source_parity=true`, `promotion_eligible=false`, canonical `blocked`. Evidence: `.codex/runs/CURRENT/master-goal/phase5/production-candidate-local` (candidate SHA-256 `A471CB2DB722D5E37130B710D59D9FF47A8F31844239D4C3064AF3B212DE13A6`, verification SHA-256 `1348B1640C3DF630D57DA29202F3076348319496ECA007D12E0B291197315EF4`); no stateful parity, deployment, registry publication, percentage increase, or provider write is implied.
