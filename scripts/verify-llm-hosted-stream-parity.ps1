...
try {
  ...
  $terminalMode = [string]$streamProof.provider_stream_terminal_mode; Require ($terminalMode -in @("provider_done_marker", "finish_reason_eof")) "stream_terminal_provenance_missing" "Independent evidence does not bind the canonical terminal marker to provider terminal evidence."
  $providerFinishReason = [string]$streamProof.provider_finish_reason
  if ($terminalMode -eq "finish_reason_eof") { Require ($providerFinishReason -match '^[A-Za-z0-9_.:-]{1,64}$') "stream_finish_reason_invalid" "Provider finish-reason EOF evidence is missing or invalid." }
