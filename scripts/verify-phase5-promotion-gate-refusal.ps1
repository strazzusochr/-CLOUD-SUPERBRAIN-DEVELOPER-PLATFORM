...
$artifactPath = "docs\release-artifacts\prod-candidate-2026-05-05-rc1-promotion-gate-refusal.md"
if (-not (Test-Path $artifactPath)) {
  throw "Missing promotion gate refusal artifact: $artifactPath"
}
...
foreach ($required in @(
  ...
  Assert-Contains "promotion gate artifact" $artifact $required
}

$candidatePath = "docs\release-artifacts\$ReleaseId.md"
$candidate = Get-Content $candidatePath -Raw
Assert-Contains "candidate promotion gate linked" $candidate "Promotion gate refusal proof: ``docs/release-artifacts/prod-candidate-2026-05-05-rc1-promotion-gate-refusal.md``"
...
Write-Host "[phase5-promotion-gate-refusal] verified"
