...
$artifactPath = "docs\release-artifacts\prod-candidate-2026-05-05-rc1-post-rollback-stability-watch.md"
if (-not (Test-Path $artifactPath)) {
  throw "Missing post-rollback stability watch artifact: $artifactPath"
}
...
foreach ($required in @(
  ...
  Assert-Contains "post-rollback stability watch artifact" $artifact $required
}

$candidatePath = "docs\release-artifacts\$ReleaseId.md"
$candidate = Get-Content $candidatePath -Raw
Assert-Contains "candidate stability linked" $candidate 'post_rollback_stability_watch_proof: `docs/release-artifacts/prod-candidate-2026-05-05-rc1-post-rollback-stability-watch.md`'
...
Write-Host "[phase5-post-rollback-stability-watch] verified"
