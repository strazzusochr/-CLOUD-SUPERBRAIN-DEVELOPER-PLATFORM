...
if ((-not $AllowLocalhost) -and ($BaseUrl -match "localhost|127\.0\.0\.1|\[::1\]")) {
  throw "Workspace pages browser proof refuses localhost unless -AllowLocalhost is set"
}
...
$nodeScript = Join-Path $PSScriptRoot "verify-workspace-pages-browser.cjs"
if (-not (Test-Path $nodeScript)) {
  throw "Missing workspace pages browser verifier"
}
...
Write-Host "[workspace-pages-browser] base url: $BaseUrl"
node @args
if ($LASTEXITCODE -ne 0) {
  throw "Workspace pages browser proof failed"
}
