...
function Assert-True($label, $condition) {
  if (-not $condition) {
    throw "Phase4 hosted request-correlation-cross-surface verification failed: $label"
  }
}
...
'@
  $payloadFile = Join-Path $env:TEMP ("phase4-request-correlation-cross-surface-" + [Guid]::NewGuid().ToString("N") + ".json")
  try {
...
function Invoke-JsonApi($url, $method = "GET", $body = $null, [hashtable]$headers = $null, $contentType = "application/json", $timeoutSeconds = 30) {
  ...
  if ([int]$response.StatusCode -ge 400) {
    throw "Phase4 hosted request-correlation-cross-surface verification failed: $method $url returned HTTP $($response.StatusCode). Value: $($response.Content)"
  }
...
function Invoke-RemoteSeedRequestCorrelationCrossSurface() {
  ...
  $localPy = Join-Path $env:TEMP ("phase4-request-correlation-cross-surface-seed-" + [Guid]::NewGuid().ToString("N") + ".py")
  $remotePy = "/tmp/phase4_request_correlation_cross_surface_seed.py"
  $script = @"
import json
import os
import subprocess
...
def task(task_id, session_id, agent_type, status, retry_count, max_retries, error, task_type):
    return {
        ...
        "task_type": task_type,
        "task_description": "hosted request correlation cross surface proof for " + task_type,
        "task_id": task_id,
        "status": status,
        "created_at": old,
        "updated_at": old,
...
with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
    with conn.cursor() as cur:
        cur.execute(
            ...
            VALUES (%s, 'hosted-phase4-request-correlation-cross-surface', 'runtime-verifier', %s)
            ON CONFLICT (id) DO NOTHING
            """,
            (project_id, Json({"source": "verify-phase4-request-correlation-cross-surface-hosted"})),
        )
        cur.execute(
            """
        ...
        cur.execute(
            ...
            (
                ...
                Json({
                    ...
                    "retry_count": 1,
                    "max_retries": 1,
                    "error": "foreign_key_violation_request_correlation_cross_surface_escalated",
                }),
                now,
                tester_session_id,
                Json({
                    ...
                    "retry_count": 0,
                    "max_retries": 1,
                    "error": "stale queued task abandoned after bounded rescue request correlation cross surface",
                }),
                now,
            ),
        )
...
client.set("task:status:" + coder_task_id, json.dumps(task(coder_task_id, coder_session_id, "coder", "escalated", 1, 1, "foreign_key_violation_request_correlation_cross_surface_escalated", "hosted_request_correlation_cross_surface_coder"), separators=(",", ":")), ex=86400)
client.set("task:status:" + tester_task_id, json.dumps(task(tester_task_id, tester_session_id, "tester", "abandoned_after_queue_drain", 0, 1, "stale queued task abandoned after bounded rescue request correlation cross surface", "hosted_request_correlation_cross_surface_tester"), separators=(",", ":")), ex=86400)
...
"@
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase4 hosted request-correlation-cross-surface verification failed: could not copy seed script" }
    $raw = ssh -i $KeyPath -o StrictHostKeyChecking=no "$RemoteUser@$StagingHost" "python3 $remotePy"
    if ($LASTEXITCODE -ne 0) { throw "Phase4 hosted request-correlation-cross-surface verification failed: remote seed execution failed" }
...
function Wait-AgentActivityCorrelation($baseUrl, $seed, $attempts = 20) {
  $last = $null
  for ($i = 0; $i -lt $attempts; $i++) {
    $last = Invoke-JsonApi -url "$baseUrl/api/v1/agent-activity/recent?limit=20&trace_id=$($seed.trace_id)" -method "GET" -contentType $null -timeoutSeconds 20
    $coder = @($last.events) | Where-Object { $_.task_id -eq $seed.coder_task_id } | Select-Object -First 1
    $tester = @($last.events) | Where-Object { $_.task_id -eq $seed.tester_task_id } | Select-Object -First 1
    if (
      $null -ne $coder -and
      $null -ne $tester -and
      $coder.request_id -eq $seed.request_id -and
      $tester.request_id -eq $seed.request_id
    ) {
      return $last
    }
    Start-Sleep -Seconds 1
  }
  throw "Phase4 hosted request-correlation-cross-surface verification failed: agent activity did not expose seeded request correlation"
}

$seed = Invoke-RemoteSeedRequestCorrelationCrossSurface
$activity = Wait-AgentActivityCorrelation -baseUrl $BaseUrl -seed $seed
...
$surfaces = @(
  @{ name = "coder activity"; item = $coderActivity },
  @{ name = "tester activity"; item = $testerActivity },
  @{ name = "coder task"; item = $coderTask },
  @{ name = "tester task"; item = $testerTask },
  @{ name = "coder session"; item = $coderSession },
  @{ name = "tester session"; item = $testerSession },
  @{ name = "coder history task"; item = $coderHistoryTask },
  @{ name = "tester history task"; item = $testerHistoryTask },
  @{ name = "coder audit"; item = $coderAudit },
  @{ name = "tester audit"; item = $testerAudit }
)

foreach ($surface in $surfaces) {
  Assert-True "$($surface.name) request_id parity" ($surface.item.request_id -eq $seed.request_id)
  Assert-True "$($surface.name) trace_id parity" ($surface.item.trace_id -eq $seed.trace_id)
  Assert-True "$($surface.name) correlation_evidence_ref parity" ($surface.item.correlation_evidence_ref -eq $seed.correlation_evidence_ref)
}

Write-Host "[phase4-request-correlation-cross-surface] base_url=$BaseUrl"
Write-Host "[phase4-request-correlation-cross-surface] trace_id=$($seed.trace_id)"
Write-Host "[phase4-request-correlation-cross-surface] request_id=$($seed.request_id)"
Write-Host "[phase4-request-correlation-cross-surface] coder_task_id=$($seed.coder_task_id)"
Write-Host "[phase4-request-correlation-cross-surface] tester_task_id=$($seed.tester_task_id)"
Write-Host "[phase4-request-correlation-cross-surface] result=verified"
