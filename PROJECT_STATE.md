...
- **P5 Cloudflare-Rebase / Scope-Audit (aktualisiert 2026-07-30):** `external-gate-audit-v2` und `external-gate-summary-v2` ersetzen die aktive RC10/Fly-Gate-Projektion atomar. Kanonische getrackte Wahrheit ist `docs/runtime-state/external-gate-audit-v2.json`, SHA-256 `0678FB8C3AD2EAA4FCC2FEB7F9124846836340FCA297529A0BD3A750799E894F`; der letzte volle lokale Lauf liegt unter `.phase1-artifacts/external-gate-audit-v2-20260730-202122.json`. Status bleibt `blocked`, `production_deploy_claim_allowed=false`; `cloudflare_native_zero_card_hosted_runtime` ist verifier-geöffnet, während exakt `github_branch_protection_current_verify` und `ghcr_image_digest_verify` fehlen. Die historische read-only CF-Inventur pruefte sechs Ressourcenfamilien ausschliesslich per GET und erhielt HTTP 401/403, Fehlercode 10000; der inzwischen qualifizierte aktive Token belegt O2Core `4/4` und O5 `1/1`, ohne R2-, Paid- oder Secret-Claim. Sanitisierter historischer Report `.codex/runs/CURRENT/p5/cloudflare-scope-readiness/report.json`, SHA-256 `DBFF2AFD4E32A0ABAD49BFD05A97E6D817F605559468DE4CD74A6BEE20CE7215`. Kein Prozentcredit oder Production-Gate-Flip. Fly/RC10-v1 ist nur `historical_only`.
...
- **Kernstand:** Localhost `8081` bleibt ausschliesslich `DEV-ONLY` Control-Plane; Gesamtfortschritt laut bindendem Manifest `86%`; Phase 1, Phase 2 und Phase 4 sind `100%`. Horizontal: P0 `100`, P1 `100`, P2 `100`, P3 `44`, P4 `100`, P5 `68`, P6 `90`. Vertikal: Frontend `100`, Orchestrator `100`, Agent Pool `69`, LLM Gateway `55`, MCP Gateway `56`, Memory `90`, Observability `100`. Cloudflare O2Core, Hosted Agent-API-Vertraege, Vercel-Origins und Gitleaks sind im aktuellen Audit positiv. Die kanonische v2-Gate-Wahrheit bleibt `blocked` wegen `github_branch_protection_current_verify` und `ghcr_image_digest_verify`; `production_deploy_claim_allowed=false`. Das stateful Runtime-Backend ist als gehosteter Kandidat verifiziert, aber Produktabnahme, Hosted-22-Seiten-Lauf und Production Release sind noch nicht bewiesen.
- Hetzner, GitKraken und Oracle sind aus dem aktiven Pfad entfernt oder als historische Altlast markiert.
- **Master Goal External-Gate-Spiegel (v2 = BLOCKED):** `docs/runtime-state/external-gate-summary.json` bindet `external-gate-summary-v2` an den getrackten Audit `docs/runtime-state/external-gate-audit-v2.json`. Beide melden `status=blocked`, `production_deploy_claim_allowed=false`, Cloudflare O2Core `claim_allowed=true` und exakt Branch Protection plus GHCR-Digest als fehlende Release-Gates. Der owner-assistierte Audit `20260713-125413` und RC10/Fly-v1 bleiben explizit nicht-aktuelle historische Provenienz.
