...
- **Aktueller gehosteter Frontend-Abschlussbeweis:** `frontend-hosted-current-proof-v1` bindet das aktive Vercel-Frontend an einen unveraenderlichen Deployment-URL- und Source-Commit-Nachweis. Echtes Google Chrome `148.0.7778.96` oeffnete alle 22 kanonischen Routen durch 44 Command-Palette-Klicks bei `1440x960` und `390x844`; Overflow, Organismus-HUD-/Gate-Kollisionen, sichtbare Not-Found-Zustaende und Console Errors blieben bei null. Immutable Deployment und Production Alias lieferten HTTP `200` und byte-identische Root-/Workspace-Wiring-Inhalte. Evidence: `.codex/runs/CURRENT/master-goal/frontend/hosted-22x2-eabdf208-chrome`, Vertrag: `docs/runtime-contracts/frontend-hosted-current.md`. Dies erhoeht Frontend von `99%` auf `100%`; Overall bleibt `84%`. Belegt ist das Frontend-Deployment, nicht der Hosted Agent API-/MCP-/LLM-Backend-Stack und nicht ein Gesamtplattform-Release.
- **Gehosteter Backend-Contract-Origin-Vorbeweis:** `backend-hosted-current-proof-v1` bindet Vercel Deployment `dpl_EVWUDmSH57FsNT6r6JdXtzTWJdK1` an Source `72e829357ed20e818f228e61af745c7fba43f445` und den SHA-256 des reinen Git-Archivs. Die authentifizierte Read-only Vercel-Abfrage belegte `READY`, korrekte Metadaten und Alias-Zuordnung; die immutable URL blieb erwartungsgemaess SSO-geschuetzt. Dieser fruehere Hosted-Snapshot meldete External Gates `6/6 verified`, liegt aber vor der R0-Quellkorrektur und ist keine aktuelle Gate-Autoritaet. Der kanonische Standard bleibt `blocked`; der Contract-Origin-Beweis muss nach Redeploy neu erzeugt werden. Der POST-Probe blieb mit `503 stateless_contract_origin_read_only` geschlossen. Evidence: `.codex/runs/CURRENT/master-goal/backend/hosted-contract-origin-72e8293/verification.json`, Vertrag: `docs/runtime-contracts/backend-hosted-current.md`. Dies ist kein stateful Docker-Backend und kein Gesamtplattform-Release.
