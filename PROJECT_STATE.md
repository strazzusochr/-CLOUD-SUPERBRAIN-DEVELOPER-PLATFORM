...
- **Session-13 Phase-5-Itemisierung / RC11 (2026-07-31):** Die verbindliche Release-Checkliste besitzt exakt `19` gleichgewichtete JA/NEIN-Items. Der eingefrorene Altstand ist damit erstmals reproduzierbar: `13/19 = 68%`, sechs benannte Luecken = gerundet `32%`. Nach E3 gilt die lokale content-addressed Verifizierbarkeit der sechs Kandidaten-Images als Release-Candidate-Readiness; GHCR-Publikation bleibt Post-Market und oeffnet keinen Gate. Die sechs eingefrorenen RC1/sslip.io-Marker sind keine Kriterien. RC11 bindet CI, Static, Runtime, Browser, Security, Clean-Archive-Images, Budget, Observability, Runbooks und offene Fragen kandidatenbezogen; `17/19 = 89%` als **berechneter**, noch nicht gutgeschriebener Wert. Nullcredit bleiben exakt I1 `hosted_candidate_parity` und I5 `production_auth_identity`. **Der Credit ist blockiert, solange der Kandidat nicht qualifiziert ist** (`mode=legacy_reconstruction`, `credit_blocked_until_candidate_qualified=true`): P5 bleibt beim rekonstruierten `68%`, Overall bei `86%`. `scripts/verify_phase5_credit_itemization.py` erzwingt Rubrik, aktiven Candidate, Source-Paritaet, Non-Claims und Manifestwert. Kein GHCR-Push, keine Promotion, kein Rollout und kein Production-Claim.
...
- **P5 Cloudflare-Rebase / Scope-Audit (aktualisiert 2026-07-31):** `external-gate-audit-v2` und `external-gate-summary-v2` ersetzen die aktive RC10/Fly-Gate-Projektion atomar. Kanonische getrackte Wahrheit ist `docs/runtime-state/external-gate-audit-v2.json`, SHA-256 `0678FB8C3AD2EAA4FCC2FEB7F9124846836340FCA297529A0BD3A750799E894F`; der letzte volle lokale Lauf liegt unter `.phase1-artifacts/external-gate-audit-v2-20260731-011557.json`. Status bleibt `blocked`, `production_deploy_claim_allowed=false`; `cloudflare_native_zero_card_hosted_runtime` und Branch Protection sind verifier-geöffnet, während nur `ghcr_image_digest_verify` fehlt. Die historische read-only CF-Inventur pruefte sechs Ressourcenfamilien ausschliesslich per GET und erhielt HTTP 401/403, Fehlercode 10000; der inzwischen qualifizierte aktive Token belegt O2Core `4/4` und O5 `1/1`, ohne R2-, Paid- oder Secret-Claim. Sanitisierter historischer Report `.codex/runs/CURRENT/p5/cloudflare-scope-readiness/report.json`, SHA-256 `DBFF2AFD4E32A0ABAD49BFD05A97E6D817F605559468DE4CD74A6BEE20CE7215`. Kein P5-Prozentcredit oder Production-Gate-Flip. Fly/RC10-v1 ist nur `historical_only`.
...
- **Kernstand:** Localhost `8081` bleibt ausschliesslich `DEV-ONLY` Control-Plane; Gesamtfortschritt laut bindendem Manifest `86%`; Phase 1, Phase 2 und Phase 4 sind `100%`. Horizontal: P0 `100`, P1 `100`, P2 `100`, P3 `44`, P4 `100`, P5 `68`, P6 `90`. Vertikal: Frontend `100`, Orchestrator `100`, Agent Pool `100`, LLM Gateway `55`, MCP Gateway `56`, Memory `100`, Observability `100`. Cloudflare O2Core, Hosted Produktabnahme, Hosted-22-Seiten-Matrix, Hosted Agent-API-Vertraege, Vectorize-Semantik, O4-Bounded-Writes, Vercel-Origins, Branch Protection und Gitleaks sind positiv. Die kanonische v2-Gate-Wahrheit bleibt nur wegen `ghcr_image_digest_verify` `blocked`; `production_deploy_claim_allowed=false`. Production OAuth, kandidatengebundene Hosted-Paritaet, Phase-6-Scale, GHCR-Publikation und Release-Promotion bleiben unbewiesen.
- Hetzner, GitKraken und Oracle sind aus dem aktiven Pfad entfernt oder als historische Altlast markiert.
- **Master Goal External-Gate-Spiegel (v2 = BLOCKED):** `docs/runtime-state/external-gate-summary.json` bindet `external-gate-summary-v2` an den getrackten Audit `docs/runtime-state/external-gate-audit-v2.json`. Beide melden `status=blocked`, `production_deploy_claim_allowed=false`, Cloudflare O2Core `claim_allowed=true`, Branch Protection `claim_allowed=true` und nur GHCR-Digest als fehlendes Release-Gate. Der owner-assistierte Audit `20260713-125413` und RC10/Fly-v1 bleiben explizit nicht-aktuelle historische Provenienz.
...
- **T5 Phase 5:** Rubrik und Verifier stehen; `17/19` sind als Score **berechnet**, aber NICHT gutgeschrieben. P5 bleibt `68%`, bis die fuenf unabhaengigen Qualifikationslaeufe (`runtime`, `browser`, `candidate_images`, `candidate_runtime`, `security`) als `passed` mit echten SHA-256 vorliegen. Nur Hosted-Candidate-Paritaet und Production OAuth bleiben Nullcredit/Owner-blockiert; GHCR-Publikation bleibt Post-Market.
...
- **Nächster sicherer Schritt:** Truth-Verifier und `verify:market-ready:static` ausführen; danach bleiben nur die im Owner-Paket benannten externen O1-, O2'-Scale- und O3-/GHCR-/Release-Wände.
