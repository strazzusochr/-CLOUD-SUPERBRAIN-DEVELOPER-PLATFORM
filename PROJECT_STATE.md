...
- **T2 freien Live-LLM-Pfad wiederherstellen:** `POST /api/v1/build` liefert hosted weiterhin ehrlich `503 llm_gateway_generation_unavailable`. Als naechsten P0-Slice einen Cloudflare Worker als echtes serverseitiges LLM Gateway implementieren, mit genau einem budgetierten Workers-AI-Miniprompt auf Preview beweisen, dann erst `LLM_GATEWAY_BASE_URL` fuer Production umstellen und erneut nachmessen. Der direkte Provider-Pfad darf nicht ins Frontend zurueckkehren.
- Danach T3/O7: L2/L3/L6 auf einen reproduzierbaren Free-Tier-Stateful-Weg statt Fly umstellen; Account-/Passwort-/Kreditkartenwaende bleiben Owner-Aktion.
