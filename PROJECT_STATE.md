...
**Active LLM Success Correlation Bundle Proof** — der aktive RC1 laeuft jetzt als neuer immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet den erfolgreichen LLM-Gateway-Pfad an Request-/Trace-/Session-Korrelation:
...
- Nachgewiesen sind hosted `overall=81`, Phase 5 `84%`, LLM Gateway `66%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
- Fortschritt steigt `phase_5=84` und `llm_gateway=66`; Gesamt bleibt `81`; kein Production-Rollout, keine Release-Promotion, kein Live-Provider-Call, kein Live-MCP-Write, kein lokaler Model-Download und keine Secret-Offenlegung.

**Active MCP Success Correlation Bundle Proof** — der aktive RC1 laeuft als immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet den erfolgreichen MCP-Gateway-Pfad an Request-/Trace-/Session-Korrelation:
...
- Nachgewiesen sind hosted `overall=81`, Phase 5 `83%`, MCP Gateway `67%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
**Active MCP Operations Bundle Proof** — der aktive RC1 laeuft jetzt als neuer immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet reale MCP-Gateway-Operations-Pfade:
...
- Nachgewiesen sind hosted `overall=81`, Phase 5 `82%`, MCP Gateway `66%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
**Active LLM Operations Bundle Proof** — der aktive RC1 laeuft jetzt als neuer immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet reale LLM-Gateway-Operations-Pfade:
...
- Nachgewiesen sind hosted `overall=81`, Phase 5 `81%`, LLM Gateway `65%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
**Active Agent Operations Bundle Proof** — der aktive RC1 laeuft jetzt als immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet reale Agent-Operations-Pfade:
...
- Nachgewiesen sind hosted `overall=80`, Phase 5 `80%`, Agent Pool `75%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
**Active Memory Operations Bundle Proof** — der aktive RC1 laeuft jetzt als immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet reale Memory-Operations-Pfade:
...
- Nachgewiesen sind hosted `overall=80`, Phase 5 `79%`, Memory `73%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
**Active Gateway Execution Bundle Proof** — der aktive RC1 laeuft jetzt als immutable Staging-Selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und bindet reale nicht-mutierende Ausfuehrungspfade:
...
- Nachgewiesen sind hosted `overall=80`, Phase 5 `79%`, `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`, GHCR `linux/arm64` fuer alle sechs Services und immutable Hetzner `-UseImageFilesystem`.
...
- Nachgewiesen sind `active_gate_count=10`, `phase5_suite_plan_status=passed`, hosted progress `80%`, Phase 5 aktuell `79%`, immutable selector `43783e702ae37ce4e88fe9f309a2488445cd83e1` und `production_rollout_claimed=false`.
...
- Hosted verifiziert auf `https://188-34-191-140.sslip.io` nach GHCR-Build/Push und immutable Image-Deploy mit `IMAGE_TAG=43783e702ae37ce4e88fe9f309a2488445cd83e1`: Active Memory Operations Bundle, Active Gateway Execution Bundle, Security-Review-Export, Browser-Contract und Hosted-Staging sind gruen.
