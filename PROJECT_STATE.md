...
- **Aktueller gehosteter Backend-Contract-Origin-Beweis:** `backend-hosted-current-proof-v1` bindet Vercel Deployment `dpl_EVWUDmSH57FsNT6r6JdXtzTWJdK1` an Source `72e829357ed20e818f228e61af745c7fba43f445` und den SHA-256 des reinen Git-Archivs. Die authentifizierte Read-only Vercel-Abfrage belegt `READY`, korrekte Metadaten und Alias-Zuordnung; die immutable URL bleibt erwartungsgemaess SSO-geschuetzt. Der oeffentliche Alias liefert `overall=84`, `P4=100`, Integritaet `verified`, External Gates `6/6 verified`, MCP/LLM `healthy` und Agent API ehrlich `degraded`. Ein POST-Probe wird mit `503 stateless_contract_origin_read_only` abgewiesen. Evidence: `.codex/runs/CURRENT/master-goal/backend/hosted-contract-origin-72e8293/verification.json`, Vertrag: `docs/runtime-contracts/backend-hosted-current.md`. Dies ist kein stateful Docker-Backend und kein Gesamtplattform-Release.
